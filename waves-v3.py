...
def display_leds_image_array():
    ...
    # Timing: 30420 ms for 1000 iterations (with leds using lists as rows)
    # Timing: 28878 ms for 1000 iterations (with leds using bytearrays as rows)
    # Timing: 28848 ms for 1000 iterations (with leds using a tuple of bytearrays as rows)
    img_array = bytearray()
    for row in leds:
        img_array.extend(row)
    # Timing: 29940 ms for 1000 iterations (with leds using a tuple of bytearrays as rows)
    #img_array = b''.join(bytes(row) for row in leds)
